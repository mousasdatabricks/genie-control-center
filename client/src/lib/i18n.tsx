...
const DICT: Record<string, Entry> = {
  ...
  'co.bySkuDesc': { pt: 'COMPUTE_SQL · COMPUTE_LLM_INFRA · COMPUTE_OTHER', en: 'COMPUTE_SQL · COMPUTE_LLM_INFRA · COMPUTE_OTHER', es: 'COMPUTE_SQL · COMPUTE_LLM_INFRA · COMPUTE_OTHER' },
  ...
  // Billing / Paygo
  'nav.billing': { pt: 'Billing Genie', en: 'Genie Billing', es: 'Billing Genie' },

  'gb.title': { pt: 'Billing Genie — visão unificada', en: 'Genie Billing — unified view', es: 'Billing Genie — vista unificada' },
  'gb.desc': { pt: 'Tudo que impacta o billing do Genie: LLM Paygo, compute por SKU, Spaces vs Code, usuário e espaço.', en: 'Everything that impacts Genie billing: LLM Paygo, compute by SKU, Spaces vs Code, user and space.', es: 'Todo lo que impacta el billing de Genie: LLM Paygo, compute por SKU, Spaces vs Code, usuario y espacio.' },
  'gb.banner': {
    pt: 'Fontes: system.billing.usage (billing_origin_product=GENIE + warehouses dos Genie Spaces) e system.access.audit. Ajuste product_surface em sql/views_prod.sql ao seu release.',
    en: 'Sources: system.billing.usage (billing_origin_product=GENIE + Genie Space warehouses) and system.access.audit. Tune product_surface in sql/views_prod.sql for your release.',
    es: 'Fuentes: system.billing.usage (billing_origin_product=GENIE + warehouses de Genie Spaces) y system.access.audit. Ajuste product_surface en sql/views_prod.sql a su release.',
  },
  'gb.sectionOverview': { pt: 'Resumo do período', en: 'Period summary', es: 'Resumen del período' },
  'gb.sectionSku': { pt: 'Por produto e SKU', en: 'By product and SKU', es: 'Por producto y SKU' },
  'gb.sectionAllocation': { pt: 'Alocação por usuário e espaço', en: 'Allocation by user and space', es: 'Asignación por usuario y espacio' },
  'gb.totalGenie': { pt: 'Custo Genie (total)', en: 'Genie cost (total)', es: 'Costo Genie (total)' },
  'gb.llmCost': { pt: 'LLM Paygo', en: 'LLM Paygo', es: 'LLM Paygo' },
  'gb.computeCost': { pt: 'Compute', en: 'Compute', es: 'Compute' },
  'gb.activeUsers': { pt: 'Usuários ativos', en: 'Active users', es: 'Usuarios activos' },
  'gb.dailyTotal': { pt: 'Custo diário total (Genie)', en: 'Total daily Genie cost', es: 'Costo diario total (Genie)' },
  'gb.dailyTotalDesc': { pt: 'Soma de todos os SKUs e categorias ligados ao Genie', en: 'Sum of all Genie-related SKUs and categories', es: 'Suma de todos los SKUs y categorías ligados a Genie' },
  'gb.byProduct': { pt: 'Uso por superfície', en: 'Usage by surface', es: 'Uso por superficie' },
  'gb.byProductDesc': { pt: 'Genie Spaces vs Genie Code vs outros', en: 'Genie Spaces vs Genie Code vs other', es: 'Genie Spaces vs Genie Code vs otros' },
  'gb.skuTable': { pt: 'Detalhe por SKU (billing)', en: 'Detail by SKU (billing)', es: 'Detalle por SKU (billing)' },
  'gb.skuTableDesc': { pt: 'sku_name real de system.billing.usage', en: 'Real sku_name from system.billing.usage', es: 'sku_name real de system.billing.usage' },
  'gb.byUser': { pt: 'Custo total por usuário', en: 'Total cost per user', es: 'Costo total por usuario' },
  'gb.byUserDesc': { pt: 'Compute rateado + LLM Paygo', en: 'Allocated compute + LLM Paygo', es: 'Compute prorrateado + LLM Paygo' },
  'gb.bySpace': { pt: 'Custo total por espaço', en: 'Total cost per space', es: 'Costo total por espacio' },
  'gb.bySpaceDesc': { pt: 'Compute rateado por perguntas + LLM quando space_id disponível', en: 'Compute allocated by questions + LLM when space_id available', es: 'Compute prorrateado por preguntas + LLM cuando space_id disponible' },
  'gb.colSku': { pt: 'SKU', en: 'SKU', es: 'SKU' },
  'gb.colCategory': { pt: 'Categoria', en: 'Category', es: 'Categoría' },
  'gb.colDbus': { pt: 'DBUs', en: 'DBUs', es: 'DBUs' },
  'gb.colCompute': { pt: 'Compute', en: 'Compute', es: 'Compute' },
  'gb.colLlm': { pt: 'LLM', en: 'LLM', es: 'LLM' },
  'gb.colTotal': { pt: 'Total', en: 'Total', es: 'Total' },
  'gb.product.GENIE_SPACES': { pt: 'Genie Spaces', en: 'Genie Spaces', es: 'Genie Spaces' },
  'gb.product.GENIE_CODE': { pt: 'Genie Code', en: 'Genie Code', es: 'Genie Code' },
  'gb.product.GENIE_OTHER': { pt: 'Genie (outros)', en: 'Genie (other)', es: 'Genie (otros)' },

  'pg.title': { pt: 'Paygo — orçamentos por usuário', en: 'Paygo — per-user budgets', es: 'Paygo — presupuestos por usuario' },
